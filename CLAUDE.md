...
1. **Enrichment** (`POST /datos` → 202, `POST /datos/sync` → 200): HubSpot search → set agente="pendiente" → Google Places text_search → Instagram via Tavily (if website is instagram.com) → TripAdvisor (optional) → **parallel optional enrichment** (website extract, Booking.com, room count, reputation — all via Tavily when available, else fallback to WebsiteScraperService/PerplexityService) → mappers → HubSpot update (with id_hotel conflict handling, auto-set cantidad_de_habitaciones + market_fit from Tavily rooms) + note + contacts → agente=""
...
- **Instagram via Tavily**: when hotel's website URL is `instagram.com`, `InstagramService` uses Tavily Extract (profile URL → raw text) with Search fallback (`include_domains=["instagram.com"]`). Regex parsing extracts name, bio, phones, email, WhatsApp, followers from the raw text. No LLM involved — Extract returns real page content. Activated when `TAVILY_API_KEY` is set. Data goes to enrichment note + creates "/ Instagram" contact.
- **Tavily integration** (`TavilyService`): uses `AsyncTavilyClient` (its own HTTP client, NOT the shared httpx — same pattern as ClaudeService). 6 methods: `extract_website` (Extract API, replaces WebsiteScraperService), `search_booking_data` (Search API with `include_domains=["booking.com"]`, replaces PerplexityService), `search_room_count` (Search API + regex for room numbers), `search_reputation` (Search API for multi-platform ratings), `extract_instagram_profile` (Extract API on Instagram URL), `search_instagram_profile` (Search API with `include_domains=["instagram.com"]`). All run in parallel via `asyncio.gather`. When `TAVILY_API_KEY` is not set, falls back to WebsiteScraperService and PerplexityService; Instagram is disabled.
- **Booking.com via Tavily/Perplexity**: Tavily preferred when available (`include_domains=["booking.com"]`). Falls back to Perplexity Sonar. `PerplexityService` still exists for Booking fallback. `app/services/booking.py` still exists but is no longer used.
...
Optional: `TRIPADVISOR_API_KEY` (empty string disables), `PERPLEXITY_API_KEY` (empty string disables Booking.com search via Perplexity), `TAVILY_API_KEY` (empty string disables; when set, Tavily is preferred over WebsiteScraperService and PerplexityService for enrichment; also enables Instagram scraping), `ELEVENLABS_API_KEY`, `ELEVENLABS_AGENT_ID`, `ELEVENLABS_PHONE_NUMBER_ID`, `ANTHROPIC_API_KEY` (empty string disables CalificarLead), `OVERWRITE_EXISTING` (default false), `LOG_LEVEL` (default INFO)
