...
1. **Enrichment** (`POST /datos` → 202, `POST /datos/sync` → 200): HubSpot search → set agente="pendiente" → Google Places text_search → TripAdvisor (optional) → website scraping (optional) → Booking.com scraping (optional) → mappers → HubSpot update + note + contacts → agente=""
